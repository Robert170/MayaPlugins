...
#include "xcExporterModel.h"
#include "xcWriterModel.h"

#include <sstream>

xcExporterModel::~xcExporterModel()
{
...
void* xcExporterModel::creator()
//Summary:  allows Maya to allocate an instance of this object
{
  return new xcExporterModel();
}


MString xcExporterModel::defaultExtension() const
...
//			MStatus::kFailure otherwise
{
  ...
  //
  status = plugin.registerFileTranslator("xcExporter",
    "",
    xcExporterModel::creator,
    "",
    "option1=1",
    true);
  if (!status) {
...
//			MStatus::kFailure otherwise
{
  ...
  status = plugin.deregisterFileTranslator("xcExporter");
  if (!status) {
...
void xcExporterModel::writeHeader(std::ostream& os)
...
WriterModel* xcExporterModel::createPolyWriter(const MDagPath dagPath, MStatus& status)
...
{
  return new xcWriterModel(dagPath, status);
}
